...
def pullCategories(course_obj_dict, filename):
    # Parses an Excel file with categorical information for each course.
    # The Excel file must be formatted with the category name (basic sciences, math, design, etc.)
    # as the first entry in eahc column, the color desired for these courses in rgb hex format (eg: ff690d)
    # as the second entry, and the courses in that category under these. Program/Technical electives must be named
    # "PROG", Complementary electives must be names "COMP", and ITS electives named "ITS".
    #
    # Arguments:
    #   course_obj_dict (dict): Stores all course data:
    #       key: Course Name (string): the Subject + " " + Catalog of a course
    #       value: Course object. Stores all data about a course
    #   filename (string): name fo the Excel file (.xls only, not .xlsx) to be parsed for category info
    #
    # Returns:
    #   course_obj_dict (dict): the "category" and "color" attributes should be filled in
    ...
    try:
        ...
        for col in range(0, sheet.ncols):
            cat_name = str(sheet.cell_value(0, col)) # category name in the first row

            if "." in str(sheet.cell_value(1, col)):
                # Excel likes to add decimal places, even though these are hex, remove this
                dotindex = str(sheet.cell_value(1, col)).find(".")
                cell_val = str(sheet.cell_value(1, col))[:dotindex]
            else:
                # No decimal places added, entry probably contains a letter
                cell_val = str(sheet.cell_value(1, col))

            # If an elective, create a new course object with name, description, category, and color attributes
            ...
            for row in range(2, sheet.nrows):
                # Formatting cell values to match with course_obj_dict key values
                name = sheet.cell_value(row, col)
                if name == "":
                    continue
                name.upper()
                name.strip()
                name.replace("  ", " ")
                if name in course_obj_dict:  # guard to prevent access error
